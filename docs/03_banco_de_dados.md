# 🗄️ MudeJá — Modelagem de Banco de Dados
## Parte 3/5 — Schema PostgreSQL Completo
...
erDiagram
    ...
    PROVIDERS ||--o{ PROPOSALS : "sends"
    SERVICE_REQUESTS ||--o{ PROPOSALS : "receives"
    USERS ||--o{ REVIEWS : "writes"
    PROVIDERS ||--o{ REVIEWS : "receives"
    PROVIDERS ||--o| SUBSCRIPTIONS : "subscribes"
    USERS ||--o{ FAVORITES : "saves"
    PROVIDERS ||--o{ FAVORITES : "is saved"
    USERS ||--o{ REPORTS : "submits"
    USERS ||--o{ NOTIFICATIONS : "receives"
    PROVIDERS ||--o{ PROVIDER_BADGES : "earns"
    PROVIDERS ||--o{ PROVIDER_SERVICE_AREAS : "covers"
    PROVIDERS ||--o{ PROVIDER_VEHICLES : "owns"
    PROVIDERS ||--o{ PROVIDER_PHOTOS : "uploads"
    SERVICE_REQUESTS ||--o{ REQUEST_ITEMS : "contains"
```
...
```sql
-- ==========================================
-- USERS (tabela base, vinculada ao Supabase Auth)
-- ==========================================
CREATE TABLE users (
...
-- ==========================================
-- PROVIDERS (perfil estendido do prestador)
-- ==========================================
CREATE TABLE providers (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL UNIQUE REFERENCES users(id) ON DELETE CASCADE,
    slug TEXT UNIQUE NOT NULL,  -- URL-friendly: "joao-mudancas-curitiba"
    business_name TEXT NOT NULL,
    description TEXT,
    short_bio TEXT,  -- max 160 chars para SEO
    ...
    -- Reputação
    trust_score DECIMAL(3,1) DEFAULT 0.0,  -- 0.0 a 10.0
    avg_rating DECIMAL(2,1) DEFAULT 0.0,   -- 0.0 a 5.0
    total_reviews INTEGER DEFAULT 0,
    total_completed INTEGER DEFAULT 0,
    total_cancelled INTEGER DEFAULT 0,
    response_rate DECIMAL(3,1) DEFAULT 0.0,    -- % de propostas respondidas
    avg_response_time INTEGER DEFAULT 0,        -- minutos
...
CREATE INDEX idx_providers_is_pro ON providers(is_pro) WHERE is_pro = TRUE;
...
-- ==========================================
-- PROVIDER_SERVICE_AREAS (áreas de atuação)
-- ==========================================
...
CREATE INDEX idx_service_areas_location ON provider_service_areas USING gist (
    ST_SetSRID(ST_MakePoint(longitude, latitude), 4326)
);
...
-- ==========================================
-- SERVICE_REQUESTS (pedidos de mudança/frete)
-- ==========================================
CREATE TABLE service_requests (
    ...
    preferred_time TEXT, -- 'manha', 'tarde', 'qualquer'
...
CREATE INDEX idx_requests_location ON service_requests USING gist (
    ST_SetSRID(ST_MakePoint(origin_longitude, origin_latitude), 4326)
);

-- ==========================================
-- REQUEST_ITEMS (itens grandes da mudança)
-- ==========================================
...
CREATE TABLE proposals (
    ...
    message TEXT,
    estimated_duration TEXT, -- "3-4 horas"
...
CREATE TABLE reviews (
    ...
    -- Moderação
    is_verified BOOLEAN DEFAULT FALSE,  -- serviço realmente aconteceu
...
-- ==========================================
-- LEAD_CREDITS (créditos de lead para plano Free)
-- ==========================================
CREATE TABLE lead_credits (
    ...
    source TEXT DEFAULT 'monthly_free',  -- 'monthly_free', 'purchased', 'bonus'
    created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE provider_badges (
    ...
    -- badge_types: 'verificado', 'top_avaliado', 'resposta_rapida', 
    --              'veterano', 'super_prestador', '100_servicos', 'pro'
...
CREATE TABLE admin_actions (
    ...
    target_type TEXT NOT NULL,  -- 'user', 'provider', 'review', 'report'
...
-- ==========================================
-- CITIES (cidades ativas na plataforma)
-- ==========================================
CREATE TABLE cities (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    name TEXT NOT NULL,
...
```sql
-- Users: podem ler seu próprio perfil, admins podem ler todos
...
-- Providers: perfis públicos são legíveis por todos
ALTER TABLE providers ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Public can read active providers" ON providers
    FOR SELECT USING (is_active = TRUE);

CREATE POLICY "Provider can update own profile" ON providers
    FOR UPDATE USING (user_id = auth.uid());

-- Service Requests: cliente vê os seus, prestadores veem os publicados
ALTER TABLE service_requests ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Client sees own requests" ON service_requests
    FOR SELECT USING (user_id = auth.uid());

CREATE POLICY "Providers see published requests" ON service_requests
    FOR SELECT USING (status IN ('published', 'receiving_proposals'));

CREATE POLICY "Client creates own requests" ON service_requests
    FOR INSERT WITH CHECK (user_id = auth.uid());

-- Proposals: prestador vê as suas, cliente vê do seu pedido
ALTER TABLE proposals ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Provider sees own proposals" ON proposals
    FOR SELECT USING (
        provider_id IN (SELECT id FROM providers WHERE user_id = auth.uid())
    );

CREATE POLICY "Client sees proposals for own requests" ON proposals
    FOR SELECT USING (
...
BEGIN
    UPDATE providers SET
        avg_rating = (
            ...
            FROM reviews WHERE reviewed_provider_id = NEW.reviewed_provider_id
            AND is_hidden = FALSE
        ),
        total_reviews = (
            SELECT COUNT(*)
...
CREATE TRIGGER trg_recalculate_ratings AFTER INSERT OR UPDATE ON reviews
    FOR EACH ROW EXECUTE FUNCTION recalculate_provider_ratings();

-- Incrementar contador de propostas no pedido
...
CREATE TRIGGER trg_increment_proposals AFTER INSERT ON proposals
    FOR EACH ROW EXECUTE FUNCTION increment_proposal_count();
```

### 4.6 Trust Score Calculation

```sql
-- Função para calcular Trust Score (0-10)
CREATE OR REPLACE FUNCTION calculate_trust_score(p_provider_id UUID)
...
BEGIN
    ...
    -- Base: avaliação média (0-2.5 pontos)
    v_score := v_score + (COALESCE(v_provider.avg_rating, 0) / 2);
    
    -- Volume de avaliações (0-1.5 pontos)
    v_score := v_score + LEAST(v_provider.total_reviews::decimal / 20, 1.5);
    
    -- Verificações (0-2.5 pontos)
    IF v_provider.selfie_verified THEN v_score := v_score + 0.5; END IF;
    IF v_provider.document_verified THEN v_score := v_score + 0.5; END IF;
    IF v_provider.whatsapp_verified THEN v_score := v_score + 0.5; END IF;
    IF v_provider.address_verified THEN v_score := v_score + 0.5; END IF;
    IF v_provider.has_insurance THEN v_score := v_score + 0.5; END IF;
    
    -- Taxa de resposta (0-1.5 pontos)
    v_score := v_score + (COALESCE(v_provider.response_rate, 0) / 100 * 1.5);
    
    -- Completude do perfil (0-1.0 ponto)
    v_score := v_score + (COALESCE(v_provider.profile_completeness, 0)::decimal / 100);
    
    -- Penalidades
    -- Cancelamentos excessivos (-0 a -1.0)
    IF v_provider.total_completed > 0 THEN
        v_score := v_score - LEAST(
            v_provider.total_cancelled::decimal / v_provider.total_completed, 1.0
        );
    END IF;
    ...
    UPDATE providers SET trust_score = ROUND(v_score, 1) WHERE id = p_provider_id;
...
> [!NOTE]
> Continua na Parte 4: Trust System, Antifraude, UX, Matching e SEO.
